    ...
    Write-Verbose "Adding tier 2 storage information into the configuration table" -Verbose
    ...
    foreach ($t2Storage in $config.storage.tier2StorageAccounts)
    {
        ...
        $customQuery = [string]::Format("(PartitionKey eq 'storage') and (storageAccountName eq '{0}')",$saName)
        ...
        {
            [hashtable]$tier2StorageProperties = @{ "resourceGroupName"=Get-ConfigValue $t2Storage.resourceGroup $config;
